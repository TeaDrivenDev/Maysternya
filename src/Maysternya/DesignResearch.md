...
Obtaining the metadata is fundamentally not difficult either: Read the `manifest.sii` file in the package determined above; if the package does not contain such a file, there will probably have to be one in an informational package. The apparently frequent lack of a display name in the manifest files is an issue; the mod IDs are just long numbers and cannot be used to identify the individual mods in the application. A possible workaround is showing (part of) the description file in place of the name; possibly with the entire file as a tooltip.
