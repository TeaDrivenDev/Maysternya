...
The contents of packages in subdirectories and zip archives can be accessed via the file system and any standard zip functionality respectively. `.scs` files can be either simply renamed zip archives,i or HashFS archives, in which case they will require an extraction tool like the one [provided by SCS](https://modding.scssoft.com/wiki/Documentation/Tools/Game_Archive_Extractor) or [this open source implementation](https://github.com/sk-zk/Extractor) to unpack. For this application, the open source extractor is more useful, as it allows targeted extraction of individual files, and for each mod there are only two small files required to be extracted.
...
If a relevant package is present as an archive, access it via .NET's built-in zip functionality to read the manifest file. If the archive cannot be read using the zip functionality, that means it is a HashFS archive; extract it to a temporary location using the open source HashFS extractor, and read the manifest file from the file system. The open source extractor is preferable to the SCS one, as it allows extracting individual files from the archive, avoiding the extraction of entire, possibly large packages just to access the small manifest and description files.  
